...
app.post('/signup',encoded,async(req,res)=>{
    ...
    student.save()
    .then(()=>{
        res.send(`<h1><strong><em>User Registration Successful!</em></strong></h1>
...
app.post('/loggedin',encoded,async(req,res)=>{
    ...
    Student1.findOne({username:username1, password:password1})
    .then(student=>{
        if(student){
            ...
            res.send(`<h1><em>Invalid Username or password</em></h1>
            <p>Click <a href="/">here</a> to sign up!</a>`)
        }
        })
        .catch((error)=>{
            console.error(error);
